...
% add a name property, show in char()
    ...
    methods
        ...
        % 'verbose'    Be verbose
            ...
            map.dim = dim;
            map.M = M; 
            map.map = [-dim+i*2*dim/size(M,2), -dim+j*2*dim/size(M,2)]'; %%% Coordinates of the features in the binery map of the environment
            map.nfeatures = size(i,1); %%% convert binary map to cartesian position 
        ...
        function f = feature(map, k)
            ...
            %
            % F = M.feature(K) is the coordinate (2x1) of the K'th map
            % feature (landmark).
            f = map.map(:,k);
        end
