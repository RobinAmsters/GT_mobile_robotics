...
classdef Roomba < handle
    ...
    methods
        ...
        function veh = Roomba(V, varargin)
        ...
        % See also Vehicle.plotv.
            ...
            h = findobj(gcf, 'Tag', 'Roomba.plot');
            if isempty(h)
                % no instance of vehicle graphical object found
                h = Vehicle.plotv(veh.x, varargin{:});
                set(h, 'Tag', 'Roomba.plot');  % tag it
            end
            
            if ~isempty(varargin) && isnumeric(varargin{1})
                % V.plot(X)
                Roomba.plotv(h, varargin{1}); % use passed value
            else
                % V.plot()
                Roomba.plotv(h, veh.x);    % use current state
            end
        end
